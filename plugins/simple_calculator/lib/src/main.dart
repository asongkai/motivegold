...
class SimpleCalculatorState extends State<SimpleCalculator> {
  ...
  List<List<GridButtonItem>> _getItems() {
    return [
      [_acLabel, Icons.arrow_back, _controller.numberFormat.symbols.PERCENT, '÷'],
      [_nums[7], _nums[8], _nums[9], '×'],
      [_nums[4], _nums[5], _nums[6], '-'],
      [_nums[1], _nums[2], _nums[3], '+'],
      [Icons.check_outlined, _nums[0], _controller.numberFormat.symbols.DECIMAL_SEP, '='],
    ].map((items) {
      return items.map((title) {
        ...
        return GridButtonItem(
          ...
          color: color,
          textStyle: style,
          child: title is String ? Text(title) : Icon(title as IconData, size: 40,)
        );
      }).toList();
    }).toList();
  }
...
getTitle(dynamic title) {
  if (title == Icons.arrow_back) {
    return 'DEL';
  }
  if (title == Icons.check_outlined) {
    return 'ENT';
  }
  return title;
}
