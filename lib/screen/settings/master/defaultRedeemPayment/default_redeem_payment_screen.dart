...
import 'package:motivegold/model/redeem_type.dart';
...
import 'package:motivegold/widget/payment/payment_default_redeem.dart';
...
class DefaultRedeemPaymentScreen extends StatefulWidget {
  const DefaultRedeemPaymentScreen({super.key});

  @override
  State<DefaultRedeemPaymentScreen> createState() => _DefaultRedeemPaymentScreenState();
}

class _DefaultRedeemPaymentScreenState extends State<DefaultRedeemPaymentScreen> {
  bool loading = false;
  List<RedeemTypeModel>? dataList = [];
  Screen? size;
  ...
  void loadData() async {
    ...
    try {
      var result = await ApiServices.post(
          '/defaultredeempayment/all', Global.requestObj(null));
      if (result?.status == "success") {
        var data = jsonEncode(result?.data);
        List<RedeemTypeModel> redeemTypes = redeemTypeModelFromJson(data);
        motivePrint(redeemTypes.first.toJson());
        setState(() {
          dataList = redeemTypes;
        });
      } else {
        dataList = [];
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(
        ...
        child: TitleContent(
          backButton: true,
          title: Text("ตั้งค่าค่าเริ่มต้น การชำระเงิน (ไถ่ถอน)",
              style: TextStyle(
                  fontSize: 28,
                  color: Colors.white,
  ...
  Widget modernPaymentCard(List<RedeemTypeModel>? list, int index) {
    return Container(
  ...
  void modernPaymentDialog(RedeemTypeModel model) {
    showDialog(
      context: context,
  ...
  void save(BuildContext context, RedeemTypeModel model) {
    model.redeemTypeId = model.id;
    ...
    DefaultRedeemPaymentModel? paymentModel;
    if (model.payment == null) {
      paymentModel = DefaultRedeemPaymentModel(
        ...
        redeemTypeId: model.redeemTypeId,
      ...
      model.payment?.redeemTypeId = model.redeemTypeId;
    ...
    Alert.info(context, 'ต้องการบันทึกข้อมูลหรือไม่?', '', 'ตกลง',
      ...
      try {
        var result = await ApiServices.post(
            '/defaultredeempayment/create', Global.requestObj(paymentModel));
        await pr.hide();
        if (result?.status == "success") {
          if (mounted) {
            loadData();
