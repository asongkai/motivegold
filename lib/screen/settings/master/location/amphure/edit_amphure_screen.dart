...
class EditAmphureScreen extends StatefulWidget {
  final AmphureModel amphure;
  final int index;

  const EditAmphureScreen(
      {super.key, required this.amphure, required this.index});

  @override
  State<EditAmphureScreen> createState() => _EditAmphureScreenState();
}

class _EditAmphureScreenState extends State<EditAmphureScreen> {
  ...
  void initState() {
    // implement initState
    super.initState();
    nameTHCtrl.text = widget.amphure.nameTh ?? '';
    nameENCtrl.text = widget.amphure.nameEn ?? '';

    var provinces =
        Global.provinceList.where((e) => e.id == widget.amphure.provinceId);
                ...
                Alert.info(context, 'ต้องการบันทึกข้อมูลหรือไม่?', '', 'ตกลง',
                  ...
                  try {
                    var result = await ApiServices.put(
                        '/location/amphure', widget.amphure.id, object);
                    await pr.hide();
