...
class AmphureScreen extends StatefulWidget {
  const AmphureScreen({super.key});

  @override
  State<AmphureScreen> createState() => _AmphureScreenState();
}

class _AmphureScreenState extends State<AmphureScreen> {
  bool loading = false;
  List<AmphureModel>? dataList = [];
  Screen? size;

  TextEditingController searchController = TextEditingController();
  List<AmphureModel> filteredList = [];
  ...
  void loadData() async {
    ...
    try {
      var result = await ApiServices.get('/location/amphure');
  ...
  Widget productCard(List<AmphureModel>? list, int index) {
    return Card(
      child: Row(
        children: [
          Expanded(
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => EditAmphureScreen(
                                amphure: list[index], index: index),
                            fullscreenDialog: true))
                        .whenComplete(() {
                      loadData();
                    });
  ...
  void remove(int id, int i) async {
    Alert.info(context, 'ต้องการลบข้อมูลหรือไม่?', '', 'ตกลง',
        action: () async {
          ...
          try {
            var result = await ApiServices.delete('/location/amphure', id, queryParams: {
