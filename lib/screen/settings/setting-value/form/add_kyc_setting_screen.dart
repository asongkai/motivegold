...
class AddKycSettingScreen extends StatefulWidget {
  const AddKycSettingScreen({super.key, this.posIdModel});

  final PosIdModel? posIdModel;

  @override
  State<AddKycSettingScreen> createState() => _AddKycSettingScreenState();
}

class _AddKycSettingScreenState extends State<AddKycSettingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        child: TitleContent(
          backButton: true,
          title: Text("เพิ่มการตั้งค่า KYC",
              style: TextStyle(
                  fontSize: 16.sp,
                  color: Colors.white,
  ...
  void _saveKycSetting() async {
    if (selectedBranch == null) {
      Alert.warning(context, 'warning'.tr(), 'กรุณาเลือกสถานประกอบการ', 'OK'.tr(),
          action: () {});
      return;
    }
    ...
    Alert.info(context, 'ต้องการบันทึกข้อมูลหรือไม่?', '', 'ตกลง',
        action: () async {
          ...
          try {
            var result = await ApiServices.post('/settings/create-kyc', object);
            if (result != null) {
              motivePrint(result.toJson());
            }
            await pr.hide();
