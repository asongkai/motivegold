...
import 'package:motivegold/screen/reports/accounting-books/paphun/refill-wholesale-paphun/preview.dart';
...
class RefillWholesalePaphunReportScreen extends StatefulWidget {
  const RefillWholesalePaphunReportScreen({super.key});

  @override
  State<RefillWholesalePaphunReportScreen> createState() =>
      _RefillWholesalePaphunReportScreenState();
}

class _RefillWholesalePaphunReportScreenState
    extends State<RefillWholesalePaphunReportScreen> {
  bool loading = false;
          ...
          title: Padding(
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  flex: 6,
                  child: Text("สมุดบัญชีซื้อทองรูปพรรณใหม่",
                      style: TextStyle(
                          fontSize: 20,
                          color: Colors.white,
    ...
    return PopupMenuButton<int>(
      onSelected: (int value) {
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => PreviewRefillWholesalePaphunReportPage(
              orders: ordersToPass,
              type: value,
  ...
  Widget _buildEnhancedDataTable() {
    ...
    for (var item in filterList!) {
      if (item?.status != "2") {
        totalPurchaseAmount += item?.priceExcludeTax ?? 0;
                                ...
                                child: IntrinsicHeight(
                                  child: Row(
                                    children: [
                                      ...
                                      // Debit - Purchase Amount total
                                      Expanded(
                                        flex: 2,
                                        child: Container(
                                          padding: const EdgeInsets.all(8),
                                          child: Text(
                                            Global.format(totalPurchaseAmount),
                                            style: TextStyle(
                                              fontWeight: FontWeight.w700,
                                              fontSize: 12,
