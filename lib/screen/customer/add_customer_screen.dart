...
class _AddCustomerScreenState extends State<AddCustomerScreen>
  ...
  Future<void> readCardWeb() async {
    print("in readecard");
    final response = await http.post(Uri.parse('http://127.0.0.1:8765/read'),
        headers: {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() async {
        ...
        firstNameCtrl.text = data['th_fullname'].toString().split(' ')[1];
        lastNameCtrl.text = data['th_fullname'].toString().split(' ').last;
        birthDateCtrl.text = data['birth'];
        Global.addressCtrl.text = data['address'];
        var province = data['address_parsed']['province'];
        var ampher = data['address_parsed']['amphoe'];
        var tambon = data['address_parsed']['tambon'];

        var result = await ApiServices.get('/location/amphure/search/$ampher');
        if (result?.status == "success") {
          print(
        ...
        setState(() {
          ...
          Global.addressCtrl.text =
              data['address'].toString().split('ตำบล').first.trim();
        });
      });
    } else {
      print("DATA Card error");
    }
  }
