...
class _SellUsedGoldScreenState extends State<SellUsedGoldScreen> {
  ...
  void initState() {
    ...
    productEntryWeightBahtCtrl.text = Global.format(434.70 / getUnitWeightValue());
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          if (selectedFromLocation == null) {
                            Alert.warning(
                                context, 'คำเตือน', 'กรุณาเลือกคลังสินค้าต้นทาง', 'OK', action: () {});
                            return;
                          }

                          if (selectedToLocation == null) {
                            Alert.warning(
                                context, 'คำเตือน', 'กรุณาเลือกคลังสินค้าปลายทาง', 'OK', action: () {});
                            return;
                          }
                          ...
                          Global.usedSellDetail!.add(
                            OrderDetailModel(
                              ...
                              commission: 0,
                              unitCost: Global.toNumber(priceIncludeTaxCtrl.text) / Global.toNumber(
                                  productEntryWeightCtrl.text),
                              priceIncludeTax:
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        if (productEntryWeightCtrl.text.isEmpty ||
                            priceIncludeTaxCtrl.text.isEmpty) {
                          Alert.warning(
                              context, 'คำเตือน', 'กรุณาเพิ่มข้อมูลก่อน', 'OK', action: () {});
                          return;
                        }
  ...
  void calTotal() {
    ...
    taxBaseTotalCtrl.text = Global.toNumber(priceDiffTotalCtrl.text) < 0
        ? "0"
        : Global.format(Global.toNumber(priceDiffTotalCtrl.text) * 100 / 107);

    taxAmountTotalCtrl.text = Global.toNumber(priceDiffTotalCtrl.text) < 0
        ? "0"
        : Global.format(
            Global.toNumber(priceDiffTotalCtrl.text) * getVatValue());
