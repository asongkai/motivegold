...
import 'preview.dart';
import 'sell_used_gold_customer_entry_screen.dart';

class SellUsedGoldCheckOutScreen extends StatefulWidget {
  const SellUsedGoldCheckOutScreen({super.key});

  @override
  State<SellUsedGoldCheckOutScreen> createState() => _SellUsedGoldCheckOutScreenState();
}

class _SellUsedGoldCheckOutScreenState extends State<SellUsedGoldCheckOutScreen> {
  int currentIndex = 1;
...
//Image Picker function to get image from camera
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('เช็คเอาท์ขายทองเก่า'.tr()),
      ),
      body: SafeArea(
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              child: Global.customer == null
                                ...
                                onPressed: () {
                                  Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                          builder: (context) =>
                                          const SellUsedGoldCustomerEntryScreen(),
                                          fullscreenDialog:
                                          true))
                                      .whenComplete(() {
                                    setState(() {});
                            ...
                            PriceBreakdown(
                              title: 'ยอดรวม'.tr(),
                              price:
                              '${Global.format(Global.getUsedSellWeightTotalAmount())} กรัม',
                            ),
                            PriceBreakdown(
                              title: ''.tr(),
                              price:
                              '${Global.format(Global.getUsedSellWeightTotalAmount() / 15.16)} บาททอง',
                            ),
                            const SizedBox(
                              height: 20,
                            ),
            ...
            child: ElevatedButton(
              ...
              onPressed: () async {
                ...
                for (var j = 0; j < Global.order!.details!.length; j++) {
                  Global.order!.priceIncludeTax = Global.order!.priceIncludeTax! + Global.order!.details![j].priceIncludeTax!;
                  Global.order!.weight = Global.order!.weight! + Global.order!.details![j].weight!;
                    ...
                    Alert.success(context, 'Success'.tr(), "", 'OK'.tr(),
                        action: () async {
                          ...
                          var result = await ApiServices.get(
                              '/sell/by-sell-id/${Global.orderIds![0]}');
                          await pr.hide();
                          if (result?.status == "success") {
                            OrderModel sell = OrderModel.fromJson(result!.data);
                            if (mounted) {
  ...
  Future postOrder() async {
    var result =
    await ApiServices.post('/sell', Global.requestObj(Global.order));
    // motivePrint(result!.data);
    if (result!.status == "success") {
      result.data['orderId'] = result.data['sellId'];
      var order = orderModelFromJson(jsonEncode(result.data));
      int? id = order.id;
      await Future.forEach<OrderDetailModel>(Global.order!.details!, (f) async {
        f.orderId = id;
        var detail =
        await ApiServices.post('/selldetail', Global.requestObj(f));
        // print(detail!.data);
