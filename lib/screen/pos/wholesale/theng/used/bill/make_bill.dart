...
Future<Uint8List> makeSellUsedThengBill(Invoice invoice) async {
  ...
  originalWidgets.add(
    await header(invoice.order, 'ใบส่งของ / ใบเสร็จรับเงิน', versionText: 'ต้นฉบับ'),
  );
  originalWidgets.add(
    docNo(invoice.order),
  );
          ...
          children: [
            Expanded(
                ...
                child: Container(
                  ...
                  child: paddedText('- ${NumberToThaiWords.convertDouble(Global.getOrderTotalWholeSale(invoice.order))} -', align: TextAlign.center),
                )),
            Expanded(
                flex: 1,
                child: Container(
  ...
  copyWidgets.add(
    await header(invoice.order, 'ใบส่งของ / ใบเสร็จรับเงิน', versionText: 'สำเนา'),
  );
  copyWidgets.add(
    docNo(invoice.order),
  );
    ...
    if (orders[i].orderTypeId == 11) {
      return Row(
        children: [
          Expanded(
            flex: 3,
            child: Text('ขายทองคำแท่งเก่า 96.5% : ',
                style: const TextStyle(fontSize: 9)),
          ),
          Expanded(
              flex: 2,
