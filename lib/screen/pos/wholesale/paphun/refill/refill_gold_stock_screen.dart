...
class _RefillGoldStockScreenState extends State<RefillGoldStockScreen> {
    ...
    AppCalculatorManager.showCalculator(
      ...
      onChanged: (key, value, expression) {
        if (key == 'ENT') {
          ...
          if (txt == 'tax_base') {
            taxBaseCtrl.text = value != null ? "${Global.format(value)}" : "";
          }
          if (txt == 'tax_amount') {
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        SizedBox(
                                            width:
                                                8), // Space between circle and text
                                        Text(
  ...
  bool _validateFields() {
    ...
    if (priceExcludeTaxCtrl.text.isEmpty) {
      Alert.warning(
          context, 'คำเตือน', 'กรุณากรอกราคารวมค่ากำเหน็จก่อนภาษี', 'OK');
      return false;
    }
