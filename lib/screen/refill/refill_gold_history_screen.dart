...
import '../products/new_product_screen.dart';
...
class _RefillGoldHistoryScreenState extends State<RefillGoldHistoryScreen> {
  ...
  void loadData() async {
    ...
    try {
      ...
      if (result?.status == "success") {
        ...
        List<RefillModel> products = refillListModelFromJson(data);
        setState(() {
          refillList = products;
        });
      } else {
  ...
  Widget dataCard(RefillModel list, int index) {
    return Card(
      child: Row(
        children: [
          Expanded(
            ...
            child: ListTile(
              title: Column(
                ...
                children: [
                  Text(
                    '#${list.refillId.toString()}',
                    style: TextStyle(fontSize: size?.getWidthPx(8)),
                  ),
                  Text(
                    Global.formatDate(list.refillDate.toString()),
                    style: TextStyle(
